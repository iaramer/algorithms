...
def func(n, pills):
    arr = defaultdict(lambda: [None, set(), set()])  # Map from difference to best value of subset sum 1
    arr[0][0] = 0  # We start with a difference of 0

    for i, pill in enumerate(pills):
        arr2 = defaultdict(lambda: [None, set(), set()])

        def add(s1, s2, idx=None, bucket=None):
            if s1 > s2:
                s1, s2 = s2, s1
            d = s2 - s1
            if d in arr2:
                arr2[d][0] = max(arr2[d][0], s1)
            else:
                arr2[d][0] = s1
            if idx is not None:
                if d in arr:
                    arr2[d][bucket] = set.union(arr2[d][bucket], arr[d][bucket])
                arr2[d][bucket].add(idx)

        for diff, sum1 in arr.items():
            sum1 = sum1[0]
            sum2 = sum1 + diff
            add(sum1, sum2)  # Ignore the current element
            add(sum1 + pill, sum2, i, 1)  # including element in partition 1
            add(sum1, sum2 + pill, i, 2)  # including element in partition 2
        arr = deepcopy(arr2)

    first, second = arr[1], arr[2]  # number of pills in the first building and in the second one
