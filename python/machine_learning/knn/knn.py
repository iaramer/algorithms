...
class KNearestNeighbor:
    ...
    def norm(X):
        norm = 2
        f = lambda x: x ** norm
        return np.sum(f(X.reshape(-1))) ** (1 / norm)
    ...
    def compute_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        square_value = np.square(X).sum(axis=1, keepdims=True) + np.square(self.X_train.T).sum(axis=0,keepdims=True) - 2*np.dot(X, self.X_train.T)
